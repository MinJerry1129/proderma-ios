...
class ClinicHomeVC: UIViewController , UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, UITableViewDataSource, UITableViewDelegate{
    
    var clinicphotoallVC : ClinicPhotoAllVC!
    var clinicinfoeditVC : ClinicInfoEditVC!
    var clinicdoctoraddVC : ClinicDoctorAddVC!
    var clinicdoctorupdateVC : ClinicDoctorUpdateVC!
    var clinicofferaddVC : ClinicOfferAddVC!
    var clinicofferdelVC : ClinicOfferDelVC!
...
//        AppDelegate.shared().currentClinicID = "1"
...
//
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        AppDelegate.shared().offerID = allOffers[indexPath.row].id
        self.clinicofferdelVC = self.storyboard?.instantiateViewController(withIdentifier: "clinicofferdelVC") as? ClinicOfferDelVC
        self.clinicofferdelVC.modalPresentationStyle = .fullScreen
        self.present(self.clinicofferdelVC, animated: true, completion: nil)
    }
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        let height = self.getRowHeightFromText(strText: allOffers[indexPath.row].description)
        return height
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: String(describing: "cell"), for: indexPath) as! ClinicDoctorCell
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        AppDelegate.shared().doctorID = allDoctors[indexPath.row].id
        self.clinicdoctorupdateVC = self.storyboard?.instantiateViewController(withIdentifier: "clinicdoctorupdateVC") as? ClinicDoctorUpdateVC
        self.clinicdoctorupdateVC.modalPresentationStyle = .fullScreen
        self.present(self.clinicdoctorupdateVC, animated: true, completion: nil)
    }
    ...
    @IBAction func onClinicImageBtn(_ sender: Any) {
        self.clinicphotoallVC = self.storyboard?.instantiateViewController(withIdentifier: "clinicphotoallVC") as? ClinicPhotoAllVC
        self.clinicphotoallVC.modalPresentationStyle = .fullScreen
        self.present(self.clinicphotoallVC, animated: true, completion: nil)
    }
    
    @IBAction func onEdtClinicInfoBtn(_ sender: Any) {
        self.clinicinfoeditVC = self.storyboard?.instantiateViewController(withIdentifier: "clinicinfoeditVC") as? ClinicInfoEditVC
        self.clinicinfoeditVC.modalPresentationStyle = .fullScreen
        self.present(self.clinicinfoeditVC, animated: true, completion: nil)
    }
    
    @IBAction func onAddDoctorBtn(_ sender: Any) {
        self.clinicdoctoraddVC = self.storyboard?.instantiateViewController(withIdentifier: "clinicdoctoraddVC") as? ClinicDoctorAddVC
        self.clinicdoctoraddVC.modalPresentationStyle = .fullScreen
        self.present(self.clinicdoctoraddVC, animated: true, completion: nil)
    }
    
    @IBAction func onAddOfferBtn(_ sender: Any) {
